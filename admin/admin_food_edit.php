...
<head>
    ...
    if (isset($_POST["upd_confirm"])) {
        $s_id = $_POST["s_id"];
        $f_id = $_POST["f_id"];
        if (isset($_POST["f_todayavail"])) {
            $f_todayavail = 1;
        } else {
            $f_todayavail = 0;
        }
        if (isset($_POST["f_preorderavail"])) {
            $f_preorderavail = 1;
        } else {
            $f_preorderavail = 0;
        }
    ...
    <div class="container form-signin mt-auto w-50">
        ...
        <form method="POST" action="admin_food_edit.php" class="form-floating" enctype="multipart/form-data">
            ...
            <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox" id="f_todayavail" name="f_todayavail" <?php if ($row["f_todayavail"]) {
                                                                                                            echo "checked";
                                                                                                        } ?>>
                <label class="form-check-label" for="f_todayavail">Menu avaliable for today</label>
            </div>
            <div class="form-check form-switch mb-2">
                <input class="form-check-input" type="checkbox" id="f_preorderavail" name="f_preorderavail" <?php if ($row["f_preorderavail"]) {
                                                                                                                echo "checked";
                                                                                                            } ?>>
                <label class="form-check-label" for="f_preorderavail">Accepting Pre-order for this menu</label>
            </div>
            <div class="form-floating mb-2">
                <input type="text" class="form-control" id="f_name" placeholder="f_name" name="f_name" value="<?php echo $row["f_name"]; ?>" required>
